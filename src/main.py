...
if __name__ == "__main__":
    ...
    sentence_delimiter = "。"
    template_start = 0
    template_end = template_start + 15
    start_ratio = 0.05
    end_ratio = 0.9
    ...
    if char_count_threshold > 0:
        translated_sentences = filter_sentences(
            "\n".join(translated_sentences), char_count_threshold, sentence_delimiter
        )
    analyze_translation(translated_sentences)

    reduced_sentences = reduce_sentences(translated_sentences, start_ratio, end_ratio)
